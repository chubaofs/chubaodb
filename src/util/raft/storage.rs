use alaya_protocol::raft::{write_action, Entry as RaftEntry, WriteActions, Kv};
use anyhow::Result;
use prost::Message;
use rocksdb::{Direction, IteratorMode, ReadOptions, WriteBatch, DB, ColumnFamily};
...
impl RaftStorage {
    ...
    pub fn iterator<'a, S: AsRef<[u8]>, V: Message + Default>(
        &'a self,
        start: S,
        end: Option<S>,
        ...
        for entry in entries {
            if let EntryDetail::Normal(actions) = &entry.detail {
                for action in actions.actions.iter() {
                    ...
                    match ty {
                        write_action::Type::Put => wb.put_cf(cf_data, key::data(&self.scope, &kv.key), &kv.value),
                        write_action::Type::Delete => wb.delete_cf(cf_data, key::data(&self.scope, &kv.key)),
                        write_action::Type::Sequence => self.update_sequence(cf_data, &mut wb, kv)?,
                    }
                }
            }
        }
...
impl RaftStorage {
    fn update_sequence(&self, cf_data: &ColumnFamily, wb: &mut WriteBatch, kv: &Kv) -> StorageResult<()> {
        use crate::util::error::Code;
        let key = key::data(&self.scope, &kv.key);
        match self.db.get_cf(cf_data, key::data(&self.scope, &kv.key))? {
            Some(v) => {
                if coding::slice_u32(&v) + 1 == coding::slice_u32(&kv.value) {
                    wb.put_cf(cf_data, key, &kv.value);
                } else {
                    return Err(Error::new(ASError::Error(Code::SequenceNotExpect, String::default())));
                }
            }
            None => {
                if coding::slice_i32(&kv.value) == 1 {
                    wb.put_cf(cf_data, key, &kv.value);
                } else {
                    return Err(Error::new(ASError::Error(Code::SequenceNotExpect, String::default())));
                }
