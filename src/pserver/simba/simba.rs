...
impl Simba {
	...
	pub async fn write(&self, req: WriteDocumentRequest) -> ASResult<()> {
		...
		match write_type {
			Some(WriteType::Put) => self._put(doc).await,
			Some(WriteType::Create) => self._create(doc).await,
	...
	async fn _put(&self, mut doc: Document) -> ASResult<()> {
		panic!()
		// let key = doc_key(&doc);
		// doc.version = 1;
		// let buf1 = self.doc_encoding(&mut doc)?;
	...
	// async fn raft_write(&self, event: Event) -> ASResult<()> {
	//     panic!()
