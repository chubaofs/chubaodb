...
impl EventCodec {
    ...
    //iid  key value
    pub fn decode<'a>(data: &'a Vec<u8>) -> (EventType, u32, &'a [u8], &'a [u8]) {
...
impl WriteRaftCallback {
    pub fn new(tx: Sender<ASError>, simba: Arc<Simba>) -> WriteRaftCallback {
        WriteRaftCallback {
            tx: tx,
            simba: simba,
        }
    ...
    fn send_result(&self, ge: ASError) {
...
impl AppendCallback for WriteRaftCallback {
    fn call(&self, cmd: &CmdResult) {
        ...
        if let Err(e) = self.simba.do_write(cmd.index, &cmd.data, false) {
            self.send_result(err_def!(e.to_string()));
        } else {
            return self.send_result(ASError::Success);
        }
    }
}
