...
impl StateMachine for NodeStateMachine {
    fn apply(&self, _term: &u64, index: &u64, command: &[u8]) -> RaftResult<()> {
    ...
    fn apply_member_change(&self, _t: CommondType, _index: u64) {
        panic!()
    }

    async fn apply_leader_change(&self, leader: u64, _term: u64, _index: u64) {
        if let Err(e) = self
            .ps
